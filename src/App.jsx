...
// Config kontrolü (Eğer placeholder kalmışsa uyarı ekranı göstermek için)
const isFirebaseConfigured =
  !!firebaseConfig &&
  typeof firebaseConfig.apiKey === "string" &&
  firebaseConfig.apiKey.trim() !== "" &&
  firebaseConfig.apiKey !== "BURAYA_GELECEK";
...
const LOGO_URL =
  "https://i.ibb.co/HppdF5nY/freepik-minimal-futuristic-gaming-logo-forge-hammer-combin-64278.png";
...
const GAMES = [
  {
    ...
    description:
      "Konuş, şüphelen, oyla: Vampirleri bul, kasabayı kurtar. Yönetici kasabayı kurar, vampir sayısını belirler ve kaderleri dağıtır; oyun gece–gündüz döngüsü ile ilerler.",
    price: "Ücretsiz Oyna",
    players: "1.2k Aktif",
  ...
  },
  {
    ...
    description:
      "Odanı kur, arkadaşlarını davet et, dijital Monopoly rekabetini başlat. Kağıt paralarla uğraşmaya son! Arkadaşlarınızla Monopoly oynarken kasanızı dijital olarak yönetin.",
    price: "Premium: 29₺",
    players: "340 Aktif",
...
const focusStyles =
  "focus:outline-none focus-visible:ring-4 focus-visible:ring-orange-500 focus-visible:ring-offset-4 focus-visible:ring-offset-slate-950";
...
function GameIcon({ iconKey, className }) {
  ...
  switch (iconKey) {
    ...
    case "message":
      return <MessageCircle className={`${cls} text-orange-500`} />;
    case "film":
...
export default function App() {
      ...
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
        <div className="bg-slate-900 border border-red-500/50 p-8 rounded-2xl max-w-lg text-center space-y-4 shadow-2xl">
          ...
          <h1 className="text-2xl font-bold text-white">
            Firebase Kurulumu Bekleniyor
          </h1>
          <p className="text-slate-400 text-sm">
            Platformu görüntüleyebilmek için <b>App.jsx</b> dosyasındaki{" "}
            <code>firebaseConfig</code> objesinin içerisine kendi Firebase
            anahtarlarını yapıştırmalısın.
          </p>
        </div>
      </div>
    );
  ...
  // ✅ Pending talepleri üstte gösterecek daha doğru sıralama
  const sortedUsers = useMemo(() => {
    const list = [...usersList].sort((a, b) => {
      const ap = a.pendingRequest ? 1 : 0;
      const bp = b.pendingRequest ? 1 : 0;
      if (ap !== bp) return bp - ap; // pending olanlar üstte
  ...
  /* ---------------------------------------------
     FIREBASE: KULLANICI (AUTH) DİNLEYİCİSİ VE CLEANUP (BUG FIX)
  ---------------------------------------------- */
  useEffect(() => {
    let unsubUserDoc = null;

    const unsubscribeAuth = onAuthStateChanged(auth, async (firebaseUser) => {
      // ✅ önceki doc listener varsa kapat (memory leak önler)
      if (unsubUserDoc) {
        unsubUserDoc();
        unsubUserDoc = null;
      }
      ...
      unsubUserDoc = onSnapshot(userRef, async (docSnap) => {
        ...
        if (docSnap.exists()) {
          ...
          // Eğer admin mail listesine sonradan eklendiyse rolünü yükseltmeyi dene
          if (isAdminEmail && userData.role !== "admin") {
            try {
              ...
              // Rules izin vermiyorsa sessiz geç.
            }
          }
  ...
  useEffect(() => {
    ...
    const unsubFeedbacks = onSnapshot(collection(db, "feedbacks"), (snapshot) => {
      const fbList = snapshot.docs.map((d) => ({ id: d.id, ...d.data() }));
  ...
  const openGame = (game) => {
    ...
    // Yakında olanlar: player overlay açıp "hazır değil" gösteriyoruz
    if (!game.url) {
      setPlayingGame(game);
      return;
    }
  ...
  // ✅ 1 oyun varsa interval kurma (gereksiz timer)
  useEffect(() => {
        ...
        <div className="flex items-center justify-between px-4 md:px-6 py-2 md:py-3 bg-slate-950 border-b border-slate-800 shadow-xl">
          <div className="flex items-center gap-2 md:gap-3 text-white font-bold truncate">
            ...
            <span className="text-sm md:text-base text-slate-300 font-medium truncate">
              Oynanıyor:{" "}
              <span className="text-emerald-400 font-bold">{playingGame.title}</span>
            </span>
          </div>
          ...
          {isLockedPremium ? (
            <div className="text-center space-y-6 p-6 max-w-md bg-slate-950 rounded-2xl border border-slate-800 shadow-2xl">
              ...
              <p className="text-slate-400">
                <b>{playingGame.title}</b> içeriğini kullanabilmek için aktif aboneliğiniz olmalıdır.
              </p>
        ...
        <div className="flex items-center gap-3 md:gap-5 shrink-0">
          ...
          {authLoading ? (
            ...
            <div className="flex items-center gap-3 pl-2 md:border-l border-slate-800">
              <div className="hidden sm:flex flex-col items-end justify-center h-full">
                ...
                {currentUser.role === "admin" ? (
                  ...
                  <span className="text-[10px] font-bold tracking-wide uppercase text-red-400">
                    Süresi Bitti
                  </span>
                ) : (
              ...
              <div
                className={`w-9 h-9 lg:w-10 lg:h-10 bg-gradient-to-br from-orange-500 to-amber-600 rounded-full flex items-center justify-center font-bold text-white shadow-lg cursor-pointer hover:ring-2 hover:ring-offset-2 hover:ring-offset-slate-950 ring-orange-500 transition-all ${
                  !isUserPremium(currentUser) && currentUser.role !== "admin" ? "animate-pulse" : ""
                }`}
                onClick={() =>
                  !isUserPremium(currentUser) && currentUser.role !== "admin" && setShowPricingModal(true)
                }
              >
            ...
            <button
              ...
              className={`flex items-center justify-center gap-2 px-4 py-2 md:px-5 md:py-2.5 bg-orange-600 hover:bg-orange-500 text-white text-sm font-bold rounded-xl transition-all shadow-lg shadow-orange-500/20 ${focusStyles} shrink-0`}
            >
    ...
    <div className="fixed inset-0 z-[200] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-md p-6 md:p-8 shadow-2xl relative">
        ...
        <form onSubmit={handleLoginSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            className="w-full flex items-center justify-center gap-2 bg-orange-600 hover:bg-orange-500 text-white font-bold py-3.5 rounded-xl transition-colors shadow-lg shadow-orange-500/20 mt-2"
          >
  ...
  /* ---------------------------------------------
     ...
     ✅ ödeme denemesi log + try/catch + lastPaymentId
  ---------------------------------------------- */
  const handlePurchaseRequest = async (plan) => {
    ...
    try {
      ...
      // 2) kullanıcı doc'unda pending + lastPaymentId yaz
      ...
      console.error(err);
      alert("Ödeme başlatılırken hata oldu. Lütfen tekrar deneyin.");
    }
  };
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-5xl p-6 md:p-10 shadow-2xl relative my-auto">
        ...
        <div className="text-center mb-8 md:mb-12 max-w-2xl mx-auto mt-4 md:mt-0">
          ...
          <p className="text-slate-400 text-sm md:text-base leading-relaxed">
            Premium abonelik ile Monopoly Dijital Banka dahil tüm araçlara anında erişim sağla.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
          ...
          <div className="bg-gradient-to-b from-orange-900/40 to-slate-950 border-2 border-orange-500 rounded-3xl p-6 md:p-8 flex flex-col transform lg:-translate-y-4 shadow-2xl shadow-orange-500/20 relative">
            <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-orange-500 text-white px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-wider shadow-lg">
              EN POPÜLER
            </div>
            <h3 className="text-2xl font-bold text-white mb-2">Sezonluk Kart</h3>
            <p className="text-orange-200/60 text-sm mb-6 h-10">
            ...
            <div className="text-orange-400 text-sm mb-6 font-bold flex items-center gap-1">
            ...
            <button
              ...
              className="w-full py-4 bg-orange-600 hover:bg-orange-500 text-white font-black rounded-xl transition-all shadow-lg shadow-orange-600/30 transform hover:scale-[1.02]"
            >
              6 Ay İçin Satın Al
            </button>
          </div>
          ...
          <div className="bg-slate-950 border border-slate-800 rounded-3xl p-6 md:p-8 flex flex-col hover:border-amber-500/50 transition-all relative">
            ...
            <button
              ...
              className="w-full py-4 bg-slate-800 hover:bg-slate-700 text-white font-bold rounded-xl transition-all border border-amber-500/30 hover:border-amber-500"
            >
              Kalıcı Destekçi Ol
            </button>
          </div>
  ...
  const handleFeedbackSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err);
      alert("Fikir gönderilirken hata oldu. Tekrar deneyin.");
    }
  };
      ...
      <div className="bg-gradient-to-r from-orange-900/30 to-slate-900 border border-orange-900/50 rounded-2xl p-6 md:p-8 flex flex-col md:flex-row items-center gap-8">
        ...
        <div className="w-full md:w-[400px] bg-slate-950 rounded-xl p-5 md:p-6 border border-slate-800 shadow-2xl shrink-0">
          <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
            <MessageSquarePlus className="w-5 h-5 text-orange-500" /> Bir Fikir Gönder
          </h3>
          <form onSubmit={handleFeedbackSubmit} className="space-y-4">
            <div>
              ...
              <select
                ...
                className="w-full bg-slate-900 border border-slate-800 rounded-lg px-3 py-2.5 text-white text-sm focus:outline-none focus:border-orange-500"
              >
                {GAMES.map((g) => (
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full bg-slate-900 border border-slate-800 rounded-lg px-3 py-2.5 text-white text-sm focus:outline-none focus:border-orange-500 resize-none"
              />
            </div>
            <button
              type="submit"
              className={`w-full flex items-center justify-center gap-2 bg-orange-600 hover:bg-orange-500 text-white font-bold py-3 rounded-lg transition-colors ${focusStyles}`}
            >
  ...
  const renderAdminDashboard = () => {
    const approvePremiumTime = async (userId, planCode) => {
      ...
      try {
        ...
        console.error(err);
        alert("Premium onaylanırken hata oldu. Firestore Rules kontrol edin.");
      }
    };
    ...
    const revokePremium = async (userId) => {
      try {
        ...
        console.error(err);
        alert("Premium iptal edilirken hata oldu. Firestore Rules kontrol edin.");
      }
    };
      ...
      <div className="space-y-6 md:space-y-8 animate-in fade-in duration-500">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
          ...
          <div className="overflow-x-auto">
              ...
              <tbody className="divide-y divide-slate-800/50">
                {sortedUsers.map((user) => {
                  ...
                  return (
                    <tr
                      key={user.id}
                      className={`hover:bg-slate-800/30 transition-colors ${isPending ? "bg-amber-900/10" : ""}`}
                    >
                      ...
                      <td className="px-6 py-4 text-center">
                        <div className="flex flex-col items-center gap-1">
                          ...
                          {isPremium && !isPending && (
                            <span className="text-[10px] text-slate-500">{remDays} gün kaldı</span>
                          )}
                        </div>
                      </td>
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-1.5 md:gap-2">
                          {isPending ? (
                            <>
                              <button
                                onClick={() => approvePremiumTime(user.id, user.pendingRequest)}
                            ...
                            <>
                              ...
                              <button
                                ...
                                className={`text-[10px] md:text-xs font-bold px-2 md:px-3 py-1.5 rounded-lg bg-orange-900/50 hover:bg-orange-800/50 text-orange-300 transition-colors ${focusStyles}`}
            ...
            {!sortedUsers.length && (
              <div className="p-6 text-center text-sm text-slate-400">Kullanıcı bulunamadı.</div>
            )}
          </div>
        </div>
      </div>
    ...
    return (
      <div className="space-y-8 md:space-y-12 animate-in fade-in duration-500">
        <section
          ...
          onKeyDown={(e) => slideList.length && handleGameKeypress(e, slideList[currentSlide])}
        >
          {!slideList.length ? (
                    ...
                    <div className="w-full lg:max-w-2xl space-y-4 md:space-y-5">
                      <div className="flex flex-wrap gap-2">
                        ...
                        {game.requiresPremium && (
                          <span className="bg-orange-600/90 backdrop-blur-sm text-white text-[10px] font-bold px-2 py-1 rounded shadow-lg flex items-center gap-1">
            ...
            <button
              ...
              className={`w-full text-left px-4 py-3 rounded-xl flex items-center gap-3 transition-colors ${
                selectedLibraryGame?.id === game.id
                  ? "bg-orange-600/20 border border-orange-500/50 text-white"
              ...
              <div className="flex items-start justify-between mb-8">
                <div>
                  ...
                  <p className="text-slate-400 text-sm md:text-base max-w-2xl leading-relaxed">
                    {selectedLibraryGame.description}
                  </p>
                </div>
